  ...
  if against_flag == True:# and len(list(G.neighbors(id))) > 15: 
    ...
    for label in range(n_arms):
      if label == arm:
        ...
        m = 0
        for j in node_neighbours:   
          if (node_outcomes[j] == 0 and arm_type[j] != -1)  and (label == model.select_without_update(gen_features.step(df_nodes.iloc[j], j)[0])):  
            m += 1
...
for iter in range(iteration_count):
  ...
  for activation_index in range(len(activation_probability)):
    ...
    for spillover_index in range(len(spillover_probability)):
      ...
      parser.add_argument('--nu', type=float, default=0.001, metavar='v', help='nu for control variance')
      parser.add_argument('--lamdba', type=float, default=0.1, metavar='l', help='lambda for regularzation')
