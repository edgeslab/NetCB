...
def generate_outcome(arm, id, spillover_index):
...
for iter in range(iteration_count):
  ...
  for activation_index in range(len(activation_probability)):
    ...
    for spillover_index in range(len(spillover_probability)):
      ...
      ee_net = EE_Net(l_spillover , h_spillover, incorrect_activation, correct_activation, total_feature_count * total_labels_count, total_labels_count, pool_step_size = 50, lr_1 = hyper_param_1, lr_2 = hyper_param_2, lr_3 = hyper_param_3,  hidden=100, neural_decision_maker = False)
      ...
      for i in tqdm(node_ID):
        ...
        if node_outcomes[i] != 1:
          ...
          r, is_arm_changed,regrets = generate_outcome(arm_select, i, spillover_index)
